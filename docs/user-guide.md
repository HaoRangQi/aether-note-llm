...
5. 回到 Hub，点 `Import`，先确认导入目标（首次默认 `私密导入`，之后会记住你上次选择）。
6. 如需切换到 `公开导入`，会弹风险确认；私密内容建议保持私密导入。
7. 第一次只粘贴一段短文本，确认导入能完成。
8. 解析完成后，会出现预览清单；可直接编辑标题、摘要、标签，并勾选要写入的条目。若检测到可能重复，预览清单会显示相似目标笔记，可选择合并、新建或丢弃。
9. 点击写入所选项后，会出现结果清单；可逐条打开生成文件或被合并的目标文件，也可以查看失败明细或撤销本次新建内容。若写入失败项被保留为待处理，可从结果页、Hub 的 `待处理` 按钮或命令面板 `Aether: 处理待导入项` 重新打开预览流继续处理。
10. 公开导入会写入 `Aether Inbox/...`，私密导入会写入 `Aether Private Inbox/...`，Hub 的“最近”区域会显示它。
