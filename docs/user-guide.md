...
**目标**：不是只找到资料，而是把过去的经验快速转成可执行方案。

1. 打开 Hub，在搜索框旁切到 `解决问题`。
2. 输入任意知识问题，例如“这个报错以前怎么处理？”。
3. Aether 会先检索当前范围内的历史笔记 / 书签，再调用 `solve` Role 输出结构化结果。
4. 结果按 `直接结论`、`推荐步骤`、`历史依据`、`风险和不确定性`、`来源引用` 展示。
5. 历史证据不足时仍会给低信心建议，但会标记缺少历史证据。
6. 点 `保存为经验卡`，在弹窗里编辑标题、问题、结论、步骤和标签，确认后写入 vault 并立即索引。

公开范围默认写入 `Aether Experience/YYYY/MM/`，私密范围默认写入 `Aether Private Experience/YYYY/MM/`。`全部` 范围会按实际引用来源选择公开或私密目录；没有引用时默认私密，避免把用户问题泄漏到公开经验目录。私密范围没有可信私密路由时，不会发送远程 solve 请求。
